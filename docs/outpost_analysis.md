...
以下内容为基于实车反馈追加的补充说明，不改动上文原始分析结论，只记录本次排查与代码修改。

### 11.1 实车现象补充

当前实车反馈为：

1. 距离大约 4 米、低弹频打弹时，识别和跟踪本身是稳定的；
2. `tracker` 可视化观测稳定，说明问题不在检测抖动或跟踪丢板；
3. 实际现象更像是：
   先命中当前装甲板 -> 随后空发一枪 -> 再命中下一块装甲板；
4. 体感上表现为“切板时机不对”，而不是“完全打不到板”。

基于这个现象，可以把问题优先收敛到 `planner` 和 `fire` 判定链路，而不是 `tracker`。

### 11.2 本次新增判断：为什么会出现“中一发、空一发”

本次复查代码后，确认有两个关键算法薄弱点：

1. `planner` 的前哨选板逻辑负责“选哪块板”，但没有把“现在是否应该开火”绑定到同一个相位判断上；
2. 当前 `plan.fire` 只看 `MPC` 是否跟上参考轨迹，没有额外确认“命中时刻这块板是否真的已经进入合适击打相位”。

这会导致一个典型问题：

1. 当前板刚打中时，`planner` 已经可能提前把目标切到下一块板；
2. 但下一块板在该时刻还没有真正进入稳定击打相位；
3. 由于 `fire` 只看轨迹误差而不看板相位，系统仍然允许开火；
4. 于是就出现了两块板之间空窗期的“空发一枪”。

进一步说，前哨当前代码里还有一个更隐蔽的问题：

1. 当前 `spin gate` 没有选到“进入窗口”的板时，会退化到 `fallback_to_closest`；
2. 这意味着在两板切换的间隙，系统仍然会提前把“最近的下一板”作为参考目标；
3. 对控制参考来说，这种做法有一定连续性；
4. 但对开火来说，如果不加额外相位约束，就很容易把空窗枪放出去。

### 11.3 本次确认并修复的算法问题

这次不是只调参数，而是明确修复了以下几个算法层面的纰漏。

#### 1. 前哨缺少“开火相位门”

原实现中：

1. 前哨选板使用 `outpost_comming_angle / outpost_leaving_angle`；
2. 但 `plan.fire` 的最终判定只使用 `MPC` 轨迹跟踪误差；
3. 缺少“选中的这块板在命中时刻是否已经进入合适相位”的检查。

修复后：

1. 前哨目标在开火前，除了需要 `tracking_error < fire_thresh`；
2. 还必须满足“选中板在命中时刻的相位角 `|delta_angle|` 落入更紧的击打窗口”；
3. 这个更紧的窗口不是新拍脑袋常数，而是基于 `outpost_leaving_angle` 推导：
   使用 `leaving_angle * 0.5`，并限制在 `4° ~ 12°` 范围内，同时不允许超过 `leaving_angle` 本身；
4. 这样做的目的，是保留切板参考的连续性，但不再允许在两板空窗期发射。

#### 2. 命中时刻迭代未收敛时，原代码仍然可能允许开火

原实现中：

1. `solve_hit_target()` 会做“选板 -> 算飞行时间 -> 预测到命中时刻 -> 再选板”的固定点迭代；
2. 即使迭代没有真正收敛，只要最后一次结果可用，后续仍然会继续规划并可能触发开火。

这在前哨这种高频切板目标上是有风险的，因为：

1. 一旦命中时刻和命中板号本身还在跳；
2. 后面的 `fire` 就可能建立在一个并不稳定的命中假设上。

修复后：

1. 对前哨目标，只有 `hit_solution.converged == true` 时才允许开火；
2. 如果命中时刻固定点迭代没有收敛，系统仍然会继续控制瞄准，但不会放枪。

#### 3. 轨迹规划起点没有优先使用“命中时刻求解出的目标板”

原实现中：

1. 命中时刻求解已经得到了一次 `hit_solution.selection`；
2. 但真正进入 `solve_aim_command()` 和 `get_trajectory()` 时，初始 `lock_id` 仍然优先沿用旧的 `lock_id_`；
3. 这会让“命中时刻求解结果”和“实际轨迹规划起点”之间存在轻微不一致的可能。

修复后：

1. 轨迹规划优先以 `hit_solution.selection.armor_id` 作为初始锁定板号；
2. 这样命中时刻求解和后续轨迹参考的起点更一致。
...
这些量已经接入 `auto_aim_debug_mpc` 和 `auto_debug` 的 plot / web state。

### 11.6 预期效果

本轮修改后，预期效果是：

1. 仍然允许 `planner` 在切板间隙维持连续参考，不至于让控制完全断掉；
2. 但不再允许把“切板空窗期”的参考目标直接当成可开火目标；
3. 低弹频条件下，原来那种“命中一发后紧跟一发空枪”的节奏应该明显减少；
4. 命中率会更偏向“少打一发，但发出去的枪更像有效枪”。
...
4. `planner_fire_phase_ready`
   如果它长期为 `false`，说明当前相位门可能偏严，或者 `delay_time` 偏差太大。

### 11.8 当前状态说明

本次按要求先不继续编译验证，因此这里记录的是：

1. 已完成代码修改；
2. 已完成文档补充；
3. 尚未做本地编译和实车回归验证。

如果后续继续推进，建议下一步按下面顺序做验证：

1. 先上车看 `planner_selected_delta_deg / planner_fire_phase_ready / planner_hit_converged`；
2. 再看空发是否已经从“固定每切板一次出现一枪”下降到“偶发”；
3. 最后再决定是否继续收紧 `delay_time` 或进一步细化前哨专用切板逻辑。
