...
参考轨迹在进入 TinyMPC 前已包含：控制延迟预测、最多 6 次命中飞行时间固定点迭代、选板/锁板、弹道 pitch 和未来 100 点采样。当前工作树还记录求解状态和迭代次数；任一轴 `tiny_solve()` 非 0 时建议避免开火。
...
6. 若两者各有优势，可采用“切板区间用五次多项式整形，正常区间继续 TinyMPC”的混合方案，但建议避免对同一加速度约束重复收紧。
...
| TinyMPC 求解状态/迭代遥测 | **已落地** | 求解失败建议避免开火；状态面板已接入 |
...
3. 弹速来源、异常回退值和弹道模型。
4. 同一段原始图像/姿态/串口记录，建议避免比较时更换数据。
5. 目标类别、装甲板尺寸、编号映射和前哨物理板高度。

建议建立至少四类回放集：静止/慢速平移、普通旋转、前哨固定旋转、遮挡/误检/重新捕获。训练式调参与最终验收数据建议分开。
...
8. **配置快照可能不一致**：当前 awakening 的部分 YAML 仍使用 `match_gate_armor/r_uv_at_1m` 等旧名字，而 `ArmorTrackerCfg::load()` 读取 `armor_match_gate`、权重和 `r_sigma_*`；建议以实际运行配置和源码版本联合验证。
...
本文对核心状态、公式和当前调用路径的置信度为 **高**；对实车收益、实际部署配置和跨硬件性能的置信度为 **低至中**，建议通过第 9 节实验补齐。
