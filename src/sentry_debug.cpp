...
DebugMode debug_mode_from_gimbal_mode(io::GimbalMode mode)
{
  switch (mode) {
    case io::GimbalMode::SMALL_BUFF:
      return DebugMode::SmallBuff;
    case io::GimbalMode::BIG_BUFF:
      return DebugMode::BigBuff;
...
nlohmann::json build_mode_state(
  DebugMode mode, io::GimbalMode serial_mode, const io::Gimbal & gimbal, bool from_web)
{
  nlohmann::json data;
  data["mode"] = static_cast<int>(mode);
  data["mode_key"] = debug_mode_key(mode);
  data["mode_label"] = debug_mode_label(mode);
  data["source"] = from_web ? "web" : "serial";
...
int main(int argc, char * argv[])
{
  ...
  if (!disable_web) {
    ...
    if (web_debugger->good()) {
      ...
      tools::logger()->info(
        "Sentry web debugger listening on {}:{} (open {})",
        web_host, web_port, web_debugger->url());
      tools::logger()->info(
  ...
  DebugMode current_mode = DebugMode::AutoAim;
  ...
  while (!exiter.exit()) {
    ...
    const bool mode_from_web = static_cast<bool>(web_debugger);
    current_mode = mode_from_web ?
      clamp_debug_mode(web_debugger->selected_mode()) :
      debug_mode_from_gimbal_mode(serial_mode);

    if (current_mode != last_mode) {
      tools::logger()->info(
        "Switch sentry debug mode: {} -> {}",
    ...
    if (current_mode == DebugMode::AutoAim) {
      ...
      io::VisionTargetState vision_target_state;
      vision_target_state.fire_permitted = current_plan.fire;
      vision_target_state.target_yaw = current_plan.target_yaw;
      vision_target_state.target_pitch = current_plan.target_pitch;
      vision_target_state.target_position_gimbal = invalid_target_point;
      ros2.publish(vision_target_state);
    ...
    if (web_debugger && now - last_web_state_time >= web_state_interval) {
      ...
      web_state["mode"] = build_mode_state(current_mode, serial_mode, gimbal, mode_from_web);
      ...
      if (current_mode == DebugMode::AutoAim) {
        ...
        web_state["planner"]["turn_sign"] = tools::debug::spin_direction_sign(current_w);
        web_state["planner"]["w_rad_s"] = current_w;
        web_state["planner"]["h_m"] = current_h;
        web_state["planner"]["selected_z_offset_m"] = planner->debug_selected_z_offset;
    ...
    if (need_visual_output) {
      ...
      if (show_local) {
        cv::imshow("Sentry Debug", display_img);
        cv::imshow("Sentry Debug Panel", ballistic_panel);
